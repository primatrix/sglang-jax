...
- Keep performance high via TPU-friendly masking kernels, grammar caching, and pluggable backends.
...
6. **TPU Optimized**: Bitmask application via JAX kernels for performance
...
**Pattern 1: Response Format (JSON Schema)**
```python
import openai
...
response = openai.chat.completions.create(
    ...
    tool_choice="required"  # or "auto", or specific tool
)
```

**Conversion Flow:**
```
OpenAI Request → ChatCompletionRequest (Pydantic model)
→ _process_messages() → Detect tool calling requirements
→ FunctionCallParser.get_structure_constraint() → structural_tag or json_schema
→ to_sampling_params() → SamplingParams
→ Backend execution
```
...
for chunk in stream:
    # Constraints applied per-token
    # Partial JSON is valid at each step
