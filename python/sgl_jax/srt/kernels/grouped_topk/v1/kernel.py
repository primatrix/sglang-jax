...
def _grouped_topk_kernel(
    ...
    *,
    ...
    #    outputs, so per-block VMEM stays O(E*BT), independent of topk.
...
def grouped_topk_pallas(
    ...
    *,
    ...
    lane dim), falling back to a single whole-batch block. `unroll` is the final-select fori_loop
    factor; default (full unroll) is fastest.
    """
