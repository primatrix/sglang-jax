    ...
    *,
    ...
    with jax.named_scope("final_select"):
        ...
        def _pick(k, carry):
            ...
            # Single argmax for the winner index (jnp.argmax/max/sum lower in Mosaic; generic
            # jax.lax.reduce with a custom combiner does NOT — "Unimplemented primitive ... reduce").
            # argmax uses the hardware tie-break (not lowest-index) — differs from ref ONLY on
            # exact-equal scores, where the tied experts are interchangeable.
