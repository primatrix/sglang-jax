    ...
    def start_a2a_scatter(*, bt_sem_id, e_sem_id, local_e_id, bt_start):
        ...
        def _scatter_one(
            ...
            for k_id in range(top_k):
                ...
                # Avoid issuing 0-byte DMA/remote copies: on some TPU platforms
                # these can trigger runtime anomalies or semaphore mismatches.
                @pl.when(local_sz > 0)
                def _(
                    *,
                    src_t_id=src_t_id,
...
def fused_ep_moe(
    ...
    *,
    ...
    print(f"block_config: {block_config}")
    ...
    fused_moe = jax.named_scope(scope_name)(
        pl.pallas_call(
            ...
            compiler_params=pltpu.CompilerParams(
                # Avoid potential collective-id collisions when multiple Pallas TPU
                # kernels with collectives appear in the same compiled program.
                collective_id=int(zlib.crc32(scope_name.encode("utf-8")) & 0x7FFFFFFF),
