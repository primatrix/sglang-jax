            ...
            def body_w1w3(bd1_idx, carry):
                ...
                for p_id in range(t_packing):
                    ...
                    if act_qdtype is not None:
                        t_q, t_scale = _quantize_fp8_per_row(t_f32, act_qdtype)
                        t_mm_in = t_q
                    else:
                        t_mm_in = t_f32
                        t_scale = None
                    ...
                    if w1_shared_scale_hbm is not None:
                        ...
                        acc_gate_part = jnp.dot(
                            t_mm_in,
                            w1_gate.astype(jnp.float32) * s_gate,
                            preferred_element_type=jnp.float32,
                        )
                    else:
                        acc_gate_part = jnp.dot(
                            t_mm_in, w1_gate.astype(jnp.float32), preferred_element_type=jnp.float32
                        )
                    if t_scale is not None:
                    ...
                    if w3_shared_scale_hbm is not None:
                        ...
                        acc_up_part = jnp.dot(
                            t_mm_in,
                            w3_up.astype(jnp.float32) * s_up,
                            preferred_element_type=jnp.float32,
                        )
                    else:
                        acc_up_part = jnp.dot(
                            t_mm_in, w3_up.astype(jnp.float32), preferred_element_type=jnp.float32
                        )
                    if t_scale is not None:
                        acc_up_part *= t_scale
