...
from sgl_jax.srt.kernels.batched_ragged_paged_attention import configs, utils

_ACTUAL_STEPS_DMA_SIZE = 128
...
class RpaSchedule:
    ...
    actual_steps: Any  # [_ACTUAL_STEPS_DMA_SIZE], logical value at index 0
    ...
    def create_shape_dtype(cls, cfgs: configs.RpaConfigs):
        ...
        idx_wrapper = SmemWrapper.create_shape_dtype((cfgs.max_steps_ub, cfgs.batch_size))
        ...
        return cls(
            ...
            dma_q=SmemWrapper.create_shape_dtype((cfgs.max_steps_ub, cfgs.batch_size, 2)),
            ...
            actual_steps=jax.ShapeDtypeStruct((_ACTUAL_STEPS_DMA_SIZE,), jnp.int32),
            cfgs=cfgs,
        )
    ...
    *,
    ...
    def k_loop(
        ...
        *,
        ...
        # With KV-share, read everything from cache. The source layer's call ran
        # earlier in this step and already wrote the current-step K/V into the
        # redirected cache slot, so the shared layer's local k/v is unused.
        kv_left_frm_cache = jnp.maximum(kv_left - q_len, 0) if update_kv_cache else kv_left
        ...
        def fill_dma_kv_new(i, dst_vmem, dma_sz, slot_start):
            ...
            if cfgs.serve.kv_layout == configs.KVLayout.SEQ_ALONG_LANE:
                ...
                new_page_start = (hbm_token_idx_base - new_tok_offset) + i * cfgs.serve.page_size
                ...
                dst_hbm = (
                    (s_idx * cfgs.serve.pages_per_seq + p_idx) << cfgs.serve.page_size_log2
                ) | p_off
    ...
    def q_loop(q_idx, _, *, s_idx, q_start, q_end, k_len, q_len, num_k):
        ...
        end_k_idx_causal = (k_len - q_len + q_idx * cfgs.bq_sz + q_sz_task - 1) // cfgs.bkv_sz + 1
...
def rpa_metadata_schedule_kernel(
    ...
    *,
    ...
    pl.debug_check(
        ...
        f"Max steps exceeded SMEM capacity limit! {max_steps} vs" f" {cfgs.max_steps_ub}",
    )
    schedule_ref.actual_steps[...] = 0
    ...
    dma_list = []
    for leaf_idx, (h, s) in enumerate(zip(flat_hbm, flat_smem)):
        if leaf_idx == len(flat_hbm) - 1:
            # ``actual_steps`` is logically scalar but physically padded so
            # older Mosaic can DMA a full aligned SMEM tile.
            write_size = _ACTUAL_STEPS_DMA_SIZE
        else:
...
def generate_rpa_metadata(
    ...
    *,
    ...
    return pl.pallas_call(
        functools.partial(rpa_metadata_schedule_kernel, cfgs=cfgs, update_kv_cache=update_kv_cache),
