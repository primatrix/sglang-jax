...
from sgl_jax.srt.kernels.batched_ragged_paged_attention import utils
...
class RpaConfigs:
    ...
    def max_steps_ub(self) -> int:
        ...
        # JAX 0.9 Mosaic requires SMEM DMA slices to be lane-tile aligned. The
        # single logical actual_steps value is therefore stored in a 128-word
        # physical buffer; only element zero is consumed.
        fixed_bytes += 128  # padded actual_steps
    ...
    def validate_inputs(
        ...
        if kv_cache.shape != expected_kv_cache_shape:
            raise ValueError(
                f"Expected {kv_cache.shape=} to be equal to" f" {expected_kv_cache_shape=}"
            )
