...
class TunableParams:
    ...
    def __ge__(self, other) -> bool:
        return (
            self.bq_sz >= other.bq_sz
            and self.bq_c_sz >= other.bq_c_sz
            and self.bkv_sz >= other.bkv_sz
            and self.batch_size >= other.batch_size
            and self.n_buffer >= other.n_buffer
        )
...
def calculate_block_sizes(
    ...
    def calculate_vmem_usage(batch_size: int, n_buffer: int, bq_sz: int, bkv_sz: int) -> int:
        ...
        if serve_cfgs.kv_layout == configs.KVLayout.SEQ_ALONG_LANE:
            bkv_array_size = (
                (bkv_sz + 2 * serve_cfgs.page_size) * aligned_num_kv_heads_x2 * aligned_head_dim
            )
        else:
    ...
    def calculate_compute_buffer_time(batch_size: int, bq_c_sz: int, bkv_sz: int) -> int:
    ...
    def find_best_block_sizes(
        ...
        while (
            batch_size > 1
            and calculate_vmem_usage(batch_size, n_buffer, bq_sz, bkv_sz) > capped_vmem_limit_bytes
        ):
            batch_size -= 1

        # As a last resort, attempt to decrease number of buffers to avoid OOM.
        while calculate_vmem_usage(batch_size, n_buffer, bq_sz, bkv_sz) > capped_vmem_limit_bytes:
            n_buffer -= 1
        ...
        while (
            calculate_compute_buffer_time(batch_size, bq_c_sz, bkv_sz) > threshold or bq_c_rem != 0
        ) and num_bq_c < bq_sz:
            if bq_c_rem == 0:
