...
from sgl_jax.srt.kernels.batched_ragged_paged_attention import configs, schedule
...
class KVBufferedRefSeqAlongLane(_BypassRef):
    ...
    cfgs: configs.RpaConfigs = dataclasses.field(default=None, metadata=dict(static=True))
    ...
    def copy_in(
        ...
        for b in range(self.cfgs.batch_size):
            for i in range(self.cfgs.bkv_p_cache):
                p_idx, dst_off, dma_valid = schedule_ref.get_dma_kv_cache(block_idx, b, i)
