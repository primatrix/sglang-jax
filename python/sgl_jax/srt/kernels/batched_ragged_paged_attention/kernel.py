...
# yapf: disable
from sgl_jax.srt.kernels.batched_ragged_paged_attention import (
    bref_override,
    configs,
    flash_attention,
    schedule,
...
def strided_load_bkv(
    ...
    *,
    ...
    kv = utils.strided_load(kv_ref, start, cfgs.bkv_sz * cfgs.bkv_stride, cfgs.bkv_stride)
    ...
    *,
    ...
    for b in range(cfgs.batch_size):
        ...
        if not cfgs.fuse_accum:
            ...
            jax.lax.cond(is_last_k, jax.named_call(_accum, name="accum"), lambda _: None, b)
        else:
            _accum(b)
...
def create_allocs(kv_cache_hbm_ref: jax.Ref, o_hbm_ref: jax.Ref, cfgs: configs.RpaConfigs) -> tuple[
    bref_override.BatchingQRef,
    bref_override.KVBufferedRefSeqAlongLane | bref_override.KVBufferedRefHeadAlongSublane,
...
def rpa_kernel(
    ...
    *,
    ...
    def ragged_paged_attention_pipeline(
        ...
        q_alloc, kv_cache_alloc, o_alloc = create_allocs(kv_cache_hbm_ref, q_hbm_ref, cfgs)

        # ``with_scoped`` was introduced after the JAX 0.9.0 runtime used by
        # SGLang-JAX. ``run_scoped`` has the same named-allocation contract and
        # executes the decorated body while tracing (the decorated name becomes
        # ``None``), so it must not be called again below.
        @functools.partial(
            pl.run_scoped,
        ...
        def _run(final_allocs, schedule_ref, dma_sem, scratches):
            ...
            # JAX 0.9 Mosaic only permits scalar loads from VMEM/SMEM. Copy the
            # physically padded actual_steps tile first, then use its logical
            # first element to form the dynamic pipeline grid.
            actual_steps_size = schedule_hbm_ref.actual_steps.shape[0]
            actual_steps_copy = pltpu.make_async_copy(
                schedule_hbm_ref.actual_steps.at[pl.ds(0, actual_steps_size)],
                schedule_ref.actual_steps.at[pl.ds(0, actual_steps_size)],
                dma_sem.at[0],
            )
            actual_steps_copy.start()
            actual_steps_copy.wait()

            actual_steps = schedule_ref.actual_steps[0]
            ...
            for h, s in zip(flat_hbm, flat_smem):
                if h.memory_space == pltpu.HBM:
                    # ``actual_steps`` was copied separately above and is the
                    # only schedule leaf smaller than one step-stride.
                    if h.shape[0] < cfgs.max_steps_ub:
                        continue
            ...
            # When performing p * v, we perform causal masking on lhs (p) by zeroing
            ...
            # since result of multiplication will be zero thanks zero on lhs. However,
            # this assumption does not hold if a row of rhs has NaNs. To avoid this,
            # we initialize scratch memory with non-zero values. Even if the scratch
