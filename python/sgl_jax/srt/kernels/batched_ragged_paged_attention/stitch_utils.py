...
from sgl_jax.srt.kernels.batched_ragged_paged_attention import configs
...
def _stitch_decode_lane(
    ...
    merged_dst_vreg = jax.lax.select(lane_idx >= dst_rel, rolled_src_vreg, dst_vreg)
...
def _stitch_prefill_lane(
    ...
    *,
    ...
    roll_shift = (bkv_sz_cache - (cache_pages * cfgs.serve.page_size + new_tok_offset)) % v_len
    rolled_u32 = pltpu.roll(vmem_u32_reshaped[...], roll_shift, axis=2)

    lane_idx = jax.lax.broadcasted_iota(jnp.int32, rolled_u32[..., : cfgs.bkv_sz].shape, 2)
...
def store_new_kv_lane(
    ...
    *,
    ...
    if cfgs.block.bq_sz == 1:
        ...
        strided_vmem_ref[pl.ds(dst_chunk_idx, outer_dim, lanes_per_col)] = merged_dst_vreg
...
def stitch_new_kv_lane(
    ...
    * page_size]
    ...
    if cfgs.block.bq_sz == 1:
        return _stitch_decode_lane(vmem_u32_ref, bkv_sz_cache, cache_pages, new_tok_offset, v_len)
    else:
