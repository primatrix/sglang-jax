...
class WeightLoader:
    ...
    @staticmethod
    def _uniform_block_dequant_kv(weight, scale, block_size):
        """Uniform block dequant: weight[out_dim, in_dim] * scale[out_blocks, in_blocks]."""
        ...
        return (w_4d * s_4d).reshape(padded_out, in_dim)[:out_dim, :].astype(jnp.bfloat16)

    def dequant_fused_kv_per_head(
        self,
        ...
        """Dequantize FP8 K+V weights with per-layer scheme detection.

        Two schemes auto-detected per layer:
        - Per-head fused: K+V quantized as fused [K(head_dim), V(v_head_dim)] per head;
          scale blocks cross K/V boundary so K and V must be dequanted together.
        ...
        After dequant, replaces attn.k_proj / attn.v_proj with bf16 LinearBase.
        """
        import math
        ...
        for layer_idx in sorted(kv_buffers.keys()):
            ...
            if is_per_head:
                ...
                k_bf16 = self._uniform_block_dequant_kv(k_weight, k_scale, block_size)
                v_bf16 = self._uniform_block_dequant_kv(v_weight, v_scale, block_size)
        ...
        kv_buffers.clear()
        logger.info("FP8 KV per-head fused dequantization complete.")
    ...
    def _handle_single_weight(
        ...
        # Handle fused K+V buffer storage (used by MiMo-V2-Flash per-head fused dequant).
