...
REQUEST_TIME_STATS_SCHEMA_VERSION = 3


def mark_time_stats(stats: dict[str, int] | None, field: str) -> None:
    """Record a sampled request's first visit to a stage."""
    if stats is not None and field not in stats:
        stats[field] = time.time_ns()


def merge_part_time_stats(parts: Iterable[dict[str, int]]) -> dict[str, int]:
    """Bound parallel parts by their earliest start and latest completion."""
    merged = {}
    for timing in parts:
        for field, value in timing.items():
            starts_stage = field.endswith(("_start_ns", "_enqueue_ns")) or field in {
                "enqueue_ns",
                "dequeue_ns",
                "receive_metadata_ns",
                "processor_submit_ns",
            }
            reduce = min if starts_stage else max
            merged[field] = reduce(merged.get(field, value), value)
    return merged


def begin_request_time_stats(raw_request: Any, request_id: str | None, server_args: Any):
    """Select the request once and retain the HTTP middleware's timestamps."""
    if raw_request is None or not server_args.enable_request_time_stats_logging:
        return None
    state = raw_request.scope.setdefault("state", {})
    if request_id is None or not should_sample_request(
        request_id, server_args.request_time_stats_sample_rate
    ):
        state[REQUEST_TIME_STATS_STATE_KEY] = None
        return None
    stats = state.get(REQUEST_TIME_STATS_STATE_KEY)
    if stats is None:
        stats = {}
    state[REQUEST_TIME_STATS_STATE_KEY] = stats
    state[REQUEST_TIME_STATS_RID_STATE_KEY] = request_id
    mark_time_stats(stats, "openai_handler_enter_ns")
    return stats
...
def mark_request_time_stats(
    ...
    for stats in stats_iterable:
        if stats is None or field in stats:
            continue
        if value is None:
            value = time.time_ns()
        stats[field] = value
...
def mark_batch_time_stats(batch: Any, field: str, timestamp_ns: int | None = None) -> None:
    ...
    mark_request_time_stats(getattr(batch, "request_time_stats", None), field, timestamp_ns)
...
class RequestTimeStatsMiddleware:
    ...
    async def __call__(
        ...
        enabled = bool(getattr(server_args, "enable_request_time_stats_logging", False))
        if (
            scope.get("type") != "http"
            or scope.get("path") != "/v1/chat/completions"
            or not enabled
            or getattr(server_args, "request_time_stats_sample_rate", 1.0) <= 0
        ):
        ...
        async def send_with_timing(message: dict[str, Any]) -> None:
            ...
            if is_first_content:
                ...
                mark_time_stats(current_stats, "server_first_content_send_done_ns")
