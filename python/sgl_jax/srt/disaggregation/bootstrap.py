...
hardening RFC. Stage 2 only needs the registration / lookup contract
the scheduler Mixin depends on.
"""
...
# Stage 4 H-D: PD wire protocol version. Bump when ``PrefillInfo``
# or any of the 4 endpoint payloads change shape. Decode rejects
# prefill workers with ``protocol_version < MIN_COMPATIBLE_VERSION``;
# the default ``MIN = PROTOCOL_VERSION - 1`` allows one minor of
# skew so rolling upgrades can proceed bootstrap → D → P.
#
# On first ship both are equal (no older protocol exists). Future
# bumps should typically advance ``PROTOCOL_VERSION`` only, leaving
# ``MIN_COMPATIBLE_VERSION`` at N-1 so a rolling upgrade with one
# minor skew remains valid.
...
class _Registry:
    ...
    def list(self) -> List[PrefillInfo]:
        with self.lock:
            self._evict_stale_locked()
            return list(self.prefills.values())

    def pick_for_room(self, bootstrap_room: int) -> Optional[PrefillInfo]:
        with self.lock:
            self._evict_stale_locked()
...
def build_app(
    ...
    *,
    ...
    ``shared_secret`` enables Stage 4 H-C Bearer auth on every
    endpoint except ``/health``; mismatched / missing tokens return
    401.
    """
...
class BootstrapServer:
    ...
    Daemon thread — process exit kills the server. Stage 2 does not
    implement graceful shutdown (left to hardening RFC).
    """
...
class BootstrapClient:
    ...
    def get_prefill_info(self, bootstrap_room: int) -> Dict[str, object]:
        ...
        r.raise_for_status()
        info = r.json()
        # Stage 4 H-D: reject peers below the supported protocol
        # floor. The bootstrap registry may legitimately contain a
        # mix during rolling upgrades, and the picker is not version
        # aware, so we defend here.
...
class HeartbeatDaemon:
    """Background thread that ``heartbeat()``s the bootstrap server on
    behalf of a prefill engine.

    Started by the scheduler entry when ``disaggregation_mode ==
    "prefill"``. Stage 2 review C2: without this, the prefill
    registration falls off the bootstrap server after
    ``HEARTBEAT_TTL_SECONDS`` and the PD path silently breaks
    ~30 s after first boot.
    """
