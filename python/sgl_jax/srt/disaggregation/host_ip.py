...
Resolution order (Stage 3 RFC §"per-host transfer server"):
...
a remote peer. We use :mod:`ipaddress` for the classification so all
IPv6 variants (long form, mixed v4/v6, etc.) are caught by category
rather than by string match (Stage 3 review I3).
"""
...
def _validate(ip: str, *, source: str) -> str:
    ...
    try:
        ...
        # Not a numeric IP — assume it's a DNS name the caller wants
        # to resolve elsewhere. Stage 3 contract requires a resolved
        # IP at this point, but be permissive: routable DNS names
        # round-trip correctly through ``f"{host}:{port}"``.
        return ip
