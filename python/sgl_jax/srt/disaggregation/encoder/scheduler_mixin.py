...
class SchedulerDisaggregationEncoderMixin:
    ...
    def process_encoder_requests(
        ...
        for pending in self.encoder_client.drain_completed():
            ...
            try:
                ...
                mark_time_stats(timing, "language_scheduler_pickup_ns")
            except Exception as exc:
                self._abort_encoder_request(recv_req, str(exc))
            else:
                ready.append(recv_req)
    ...
    def _mark_encoder_batch(self: Scheduler, batch: ScheduleBatch, field: str) -> None:
        if self.server_args.enable_request_time_stats_logging and batch.forward_mode.is_extend():
            mark_request_time_stats(
                (
                    getattr(req, "encoder_timing", None)
