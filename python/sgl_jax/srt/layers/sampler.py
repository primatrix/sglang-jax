...
def multinomial_with_seed(
    ...
    n, m = inputs.shape
    # step_seed = (seed.astype(jnp.uint64) * jnp.uint64(19349663)) ^ (positions.astype(jnp.uint64) * jnp.uint64(73856093))
    seed_expanded = seed[:, None]
    col_indices = jnp.arange(m, dtype=jnp.uint64)[None, :]
