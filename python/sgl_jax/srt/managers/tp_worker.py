...
class ModelWorker:
    ...
    def generate_model_worker_batch(
        ...
        return ModelWorkerBatch(
            bid=1,
            forward_mode=mode,
            input_ids=np.concatenate([valid_input_ids, invalid_input_ids], axis=0),
            real_input_ids_len=len(valid_input_ids),
            real_bs=bs,
            req_pool_indices=np.arange(bs, dtype=np.int32),
            seq_lens=np.array([1] * bs, dtype=np.int32),
            out_cache_loc=np.concatenate([valid_out_cache_loc, invalid_out_cache_loc], axis=0),
            ...
            positions=np.concatenate([valid_positions, invalid_positions], axis=0),
            extend_start_loc=np.arange(bs, dtype=np.int64),
            cache_loc=np.concatenate([valid_cache_loc, invalid_cache_loc], axis=0),
