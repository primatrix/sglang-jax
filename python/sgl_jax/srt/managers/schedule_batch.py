...
class ScheduleBatch:
    ...
    def retract_decode(self, server_args: ServerArgs) -> tuple[list[Req], float, list[Req]]:
        ...
        - If a rank's last remaining request still won't fit, aborts it gracefully
          instead of crashing the scheduler.

        Returns:
            retracted_reqs: All retracted requests
            new_estimate_ratio: Updated estimate
            reqs_to_abort: Requests that were aborted due to OOM (per-rank corner case)
        """
        ...
        for dp_rank in range(self.dp_size):
            ...
            if len(sorted_indices) <= 1 and not has_sufficient_memory(dp_rank, sorted_indices):
                if len(sorted_indices) == 1:
                    ...
                    logger.warning(
                        "retract_decode[dp=%d]: aborted last request %s due to OOM",
                        dp_rank,
                        last_req.rid,
                    )
