...
class TokenizerManager:
    ...
    def _create_tokenized_object(
        ...
        # PD disaggregation passthrough (Stage 2). When the engine is
        # running in disaggregation_mode=decode, the request body MUST
        # carry bootstrap_{host,port,room}; reject early instead of
        # propagating a half-populated tokenized object to the
        # scheduler.
        #
        # Stage 3 router integration: if the request didn't carry
        # bootstrap_* fields but the engine knows its bootstrap URL,
        # auto-derive them so the router doesn't need to know about
        # PD. ``bootstrap_room`` is a stable 32-bit hash of the
        # request id so a retry of the same rid lands on the same P.
        if (
            ...
            if (
                ...
                if parsed.hostname is not None and parsed.port is not None:
                    if obj.bootstrap_host is None:
                        # urlparse strips IPv6 brackets; restore them so
                        # downstream ``f"{host}:{port}"`` builders
                        # produce a parseable URL. Stage 3 review I1.
            ...
            if obj.bootstrap_room is None and obj.rid is not None:
                # zlib.crc32 on Python 3 already returns an unsigned
                # 32-bit int; the explicit & 0xFFFFFFFF earlier was
                # redundant (Stage 3 review M2).
