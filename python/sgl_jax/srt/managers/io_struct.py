...
class TokenizedGenerateReqInput:
    ...
    # PD disaggregation routing keys (Stage 2). Required when the
    # tokenizer is running in disaggregation_mode=decode; the values
    # are passed through from the HTTP request to the scheduler so the
    # decode-side KVReceiver can resolve the prefill peer via the
    # bootstrap server.
...
class GenerateReqInput:
    ...
    # PD disaggregation routing keys (Stage 2). Surface on the HTTP
    # request body; the tokenizer validates and passes them through to
    # the scheduler as fields on TokenizedGenerateReqInput.
    ...
    def __getitem__(self, i):
        return GenerateReqInput(
            ...
            # PD disaggregation passthrough (Stage 2 review M5).
            # Supports both scalar (broadcast to all items) and list
            # (per-item) shapes so a router can route batched requests.
            bootstrap_host=(
