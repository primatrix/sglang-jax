...
def _mha_sequence(
    ...
    *,
    ...
    def _fetch_bq(target_seq_idx, bq_idx, sem_idx, *, wait=False):
        ...
        copy_size = bq_size
        if use_dynamic_dma:
            # Token is a sublane-tiled dim in the head-major buffer, so the DMA
            # slice must be a multiple of 8. Round up (over-read <=7 rows past
            # the sequence end land in bucket slack and are masked out).
            copy_size = jnp.minimum(bq_size, cu_lens_ref[target_seq_idx + 1] - start)
            copy_size = pl.multiple_of(((copy_size + 7) // 8) * 8, 8)
