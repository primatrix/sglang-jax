...
class VisionFlashAttentionBackend(AttentionBackend):
    ...
    def __call__(self, q, k, v, metadata: VisionAttentionMetadata):
        ...
        # Pad T to the tile this layer's kernel path wants: window layers run the
        # single-step kernel (needs T >= 256); full-frame layers run block-sparse
        # (needs T % 128, and a 2048 tile once long enough to prefer 512 blocks).
        if not self.block_sparse_segments:
            alignment = 256
