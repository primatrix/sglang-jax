...
class Qwen2_5_VL_Generation(nnx.Module):
    ...
    def get_input_embeddings(self, input_ids: jax.Array, vision_embeds: jax.Array) -> jax.Array:
        ...
        if self.video_token_id is not None:
            ...
            mask = video_mask if mask is None else (mask | video_mask)

        if mask is None:
            return text_embeds

        num_mm = vision_embeds.shape[0]
        if num_mm == 0:
            return text_embeds

        mm_positions = jnp.nonzero(mask, size=num_mm, fill_value=0)[0]
        return text_embeds.at[mm_positions].set(vision_embeds)
    ...
    def __call__(
        ...
        input_embeds = (
            self.get_input_embeddings(forward_batch.input_ids, vision_embeds)
