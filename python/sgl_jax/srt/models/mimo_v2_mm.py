...
encode glue (lane packing, bucketed metadata, ``encode_items`` →
``PackedMultimodalEmbedding``) follows the same shape contract as
``qwen2_5_vl.py`` so the generic merge in ``host_orchestration`` applies
unchanged.

Audio is not wired in this module yet (vision-first); an audio tower can be
added alongside following the same ``encode_items`` contract.
"""
...
from sgl_jax.srt.multimodal.in_model.lane_packing import pack_batch, to_packed_embedding
...
class MiMoVisionTransformer(nnx.Module):
    ...
    def __call__(self, patches, meta: _MiMoVisionMetadata, valid) -> jax.Array:
    ...
    def _pack_metadata(self, items: list[MultimodalDataItem]) -> _MiMoVisionMetadata:
        col_indices, freqs = [], []
        cu_seqlens = [0]
        unit_offset = 0
        patch_offset = 0
        for item in items:
            ...
            patch_offset += int(item.feature.shape[0])
    ...
    def _num_lanes(self) -> int:
        ...
        return data * (1 if self.vision_tp else tensor)

    def _put_batch(self, value: np.ndarray) -> jax.Array:
        if self.mesh is None:
            return jnp.asarray(value)
        return jax.device_put(value, self.specs.sharding(self.specs.batch_axis))
    ...
    def _batch_items(self, items: list[MultimodalDataItem]):
        return pack_batch(
            ...
            pack_metadata=self._pack_metadata,
            empty_metadata=self._empty_metadata,
            pad_metadata=self._pad_metadata,
        )
    ...
    def encode(self, patches, meta, valid) -> jax.Array:
        if self.mesh is None:
    ...
    def encode_items(self, items: list[MultimodalDataItem]) -> PackedMultimodalEmbedding:
        ...
        patches, metadata, valid, placements = self._batch_items(items)
    ...
    def precompile(self) -> None:
        for capacity in self.input_buckets:
            ...
            patches, metadata, valid, _ = self._batch_items([item])
...
class MiMoAudioEncoder(nnx.Module):
    ...
    def _num_lanes(self) -> int:
        ...
        data = int(self.mesh.shape.get("data", 1))
        tensor = int(self.mesh.shape.get("tensor", 1))
        return data * (1 if self.encoder_tp else tensor)
    ...
    def encode_items(self, items: list[MultimodalDataItem]) -> PackedMultimodalEmbedding:
        if not items:
            return PackedMultimodalEmbedding(jnp.zeros((0, 0, 0), jnp.float32), ())
        codes, valid, placements = self._batch_items(items)
        output = self.encode(codes, valid)
        return to_packed_embedding(output, placements, self.mesh)
    ...
    def precompile(self) -> None:
        for capacity in self.input_buckets:
            item = MultimodalDataItem(
                Modality.AUDIO,
                feature=np.zeros((capacity, self.channels), dtype=np.int32),
            )
            codes, valid, _ = self._batch_items([item])
...
class _MiMoV2MultimodalMixin(InModelMultimodalContract):
    ...
    def get_multimodal_embedding_packed_shapes(self) -> tuple[tuple[int, int], ...]:
        ...
        if self.visual is not None:
            ...
            shapes.extend((rows, bucket // unit) for bucket in self.visual.input_buckets)
        if self.audio_encoder is not None:
            rows = self.audio_encoder._num_lanes()
            group = self.audio_encoder.group_size
            shapes.extend((rows, bucket // group) for bucket in self.audio_encoder.input_buckets)
        return tuple(shapes)
    ...
    def get_multimodal_encode_funcs(self):
        ...
        if self.visual is not None:
            ...
            funcs[Modality.IMAGE] = encode
            funcs[Modality.MULTI_IMAGES] = encode
            funcs[Modality.VIDEO] = encode
        if self.audio_encoder is not None:
            funcs[Modality.AUDIO] = self.audio_encoder.encode_items
        return funcs
