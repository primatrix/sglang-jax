...
class MiMoVisionTransformer(nnx.Module):
    ...
    def encode(self, patches, grid_thw: np.ndarray | jax.Array) -> jax.Array:
        ...
        meta = put_sharded_batch(meta, self.mesh, self.specs.batch_axis)
        valid = put_sharded_batch(valid, self.mesh, self.specs.batch_axis)
        if self.mesh is None:
...
class _MiMoV2MultimodalMixin(InModelMultimodalContract):
    ...
    def get_audio_feature(self, items: list[MultimodalDataItem]) -> jax.Array:
        ...
        output = encoder.encode(
            put_sharded_batch(packed.features, self.mesh, encoder.specs.batch_axis),
            put_sharded_batch(packed.valid, self.mesh, encoder.specs.batch_axis),
        )
