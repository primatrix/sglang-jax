...
class _MiMoVisionMetadata:
    ...
    def __init__(self, col_index, reverse_col_index, rotary_freqs, cu_seqlens):
        self.col_index = col_index
        self.reverse_col_index = reverse_col_index
...
class MiMoVisionTransformer(nnx.Module):
    ...
    def _metadata_for_grid(self, grid: tuple[int, int, int]) -> _MiMoVisionMetadata:
        ...
        meta = _MiMoVisionMetadata(col_index, reverse_col_index, freqs, cu_seqlens)
    ...
    def _pack_metadata(self, grids: list[tuple[int, int, int]]) -> _MiMoVisionMetadata:
        ...
        return _MiMoVisionMetadata(
            ...
            np.concatenate(reverse_col_indices),
...
class _MiMoV2MultimodalMixin(InModelMultimodalContract):
    ...
    def load_weights(self, model_config: ModelConfig) -> None:
        # Static-checkpoint preparation runs before this hook and applies the
        # text FP8 rule to every LinearBase in the graph.  Vision/audio weights
        # are BF16, so reconstruct any tower that was converted accidentally.
        with jax.set_mesh(self.mesh):
            if self.visual is not None and not isinstance(self.visual.merger.mlp_fc1, LinearBase):
                logger.info("Restoring MiMoV2 vision tower to its BF16 structure.")
                self.visual = MiMoVisionTransformer(
                    ...
                    self.mesh,
                    self.encoder_tp,
                    self.visual.input_buckets,
                )
            if self.audio_encoder is not None and not isinstance(
                self.audio_encoder.proj_fc1, LinearBase
            ):
                logger.info("Restoring MiMoV2 audio tower to its BF16 structure.")
        ...
        # The text loader updates the model graph from its state.  Keep the
        # independently loaded multimodal towers out of that graph update so
        # it cannot drop or mutate their BF16 structure.
        visual = self.visual
