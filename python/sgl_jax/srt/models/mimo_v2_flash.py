...
class MiMoV2FlashForCausalLM(nnx.Module):
    ...
    def __init__(
        ...
        # Populated during weight loading by WeightLoader's __KV_* hook,
        # consumed by loader.dequant_fused_kv_per_head.
    ...
    def load_weights(self, model_config: ModelConfig):
        ...
        if self.loader.is_static_quant:
            ...
            block_size = int(self._quant_config.weight_block_size[0]) if self._quant_config else 128
            # Q only: K/V go through per-head fused dequant below
            self.loader.dequant_fp8_layers(
                self.model.layers,
                specs=[("self_attn.q_proj", head_dim)],
            )
            # K+V: per-head fused dequant (cross K/V boundary blocks)
            self.loader.dequant_fused_kv_per_head(
                self._kv_buffers, self.model.layers, self.config, block_size=block_size
            )
            # Layer 0 dense MLP
            self.loader.dequant_fp8_layers(
                self.model.layers,
    ...
    def _create_layer_mappings(self, layer_idx: int) -> dict:
        ...
        for proj, sharding, kv_pad, hd_pad in [
            ...
            # FP8 K/V: route raw weight+scale to __KV_* buffers for fused
