    ...
    def __init__(
        ...
        if layer_id <= first_k_dense_replace:
            ...
            self.moe_gate = GateLogit(
                ...
                model_name=getattr(config, "model_name", "bailing_moe"),
                use_bias=getattr(config, "moe_router_enable_expert_bias", False),
...
class BailingMoEForCausalLM(nnx.Module):
    ...
    def _create_moe_layer_mappings(self, layer_idx: int, is_mlp_layer: bool) -> dict:
        ...
        if is_mlp_layer:
            ...
            mappings[f"{prefix}.mlp.gate.weight"] = (
                WeightMapping(
            ...
            if getattr(self.config.hf_config, "num_shared_experts", 0) > 0:
                shared_experts_mappings = {
                    f"{prefix}.mlp.shared_experts.gate_proj.weight": WeightMapping(
                        target_path=f"{target_prefix}.mlp.shared_experts.gate_proj.weight",
                        sharding=(None, "tensor"),
                        transpose=True,
                    ),
                    f"{prefix}.mlp.shared_experts.up_proj.weight": WeightMapping(
                        target_path=f"{target_prefix}.mlp.shared_experts.up_proj.weight",
                        sharding=(None, "tensor"),
                        transpose=True,
                    ),
                    f"{prefix}.mlp.shared_experts.down_proj.weight": WeightMapping(
                        target_path=f"{target_prefix}.mlp.shared_experts.down_proj.weight",
                        sharding=("tensor", None),
                        transpose=True,
                    ),
                }
