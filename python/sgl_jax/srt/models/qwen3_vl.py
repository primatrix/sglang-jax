...
class Qwen3VLVisionModel(nnx.Module):
    ...
    def precompile(self) -> None:
        for capacity in self.input_buckets:
            ...
            batch = self._batch_items([item])
            jax.block_until_ready(self.encode(*batch[:-1]))

    def encode_items(self, items: list[MultimodalDataItem]) -> PackedMultimodalEmbedding:
        """
        The merge gathers directly from ``output`` [num_lanes, cap, (1+D)*H] --
        primary embedding plus D deepstack planes concatenated on the feature
        axis -- whose row dim is a mesh constant and cap dim is bucketed ->
        bounded compilation, no per-image-size churn.
        """
        if not items:
            return PackedMultimodalEmbedding(jnp.zeros((0, 0, 0), jnp.float32), ())
        *inputs, placements = self._batch_items(items)
        output = self.encode(*inputs)
        return to_packed_embedding(output, placements, self.mesh, len(self.deepstack_indexes))

    def _put_batch(self, value):
        """Move a host array (or a pytree such as the vision metadata) to the
        batch sharding; static pytree leaves pass through untouched."""
    ...
    def _num_lanes(self) -> int:
        ...
        return data_size * (1 if self.tp else tensor_size)
    ...
    def _batch_items(self, items: list[MultimodalDataItem]):
        ...
        features = self._put_batch(packed.features)
        lane_metadata = []
        for lane in packed.lanes:
            grids = []
            for item_index in lane:
                grid = _grid(items[item_index])
                t, h, w = grid
                if min(grid) <= 0 or h % self.merge or w % self.merge:
                    raise ValueError(f"invalid vision grid {grid} for spatial merge {self.merge}")
                if t * h * w != items[item_index].feature.shape[0]:
                    raise ValueError(
                        f"vision grid {grid} does not match "
                        f"{items[item_index].feature.shape[0]} input patches"
                    )
                grids.append(grid)
            lane_metadata.append(self._lane_metadata(grids, packed.cap))
        pos_indices, pos_weights, position_ids, cu_seqlens = (
            np.stack(values) for values in zip(*lane_metadata, strict=True)
        )
        # Single block-diagonal (full-frame) layout, planned on the host.
        metadata = VisionAttentionMetadata(cu_seqlens)
        return (
            features,
            self._put_batch(pos_indices),
            self._put_batch(pos_weights),
            self._put_batch(position_ids),
            self._put_batch(metadata),
            packed.placements,
        )
