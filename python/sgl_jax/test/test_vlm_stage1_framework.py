...
def test_qwen3_vision_precompile_warms_configured_buckets():
    ...
    assert _assert_vision_precompile(visual) == [
        ((1, 4, 1), (1, 4, 4), (1, 4, 4), (1, 4, 2), (1, 2)),
        ((1, 8, 1), (1, 4, 8), (1, 4, 8), (1, 8, 2), (1, 3)),
    ]
...
def test_block_sparse_vision_backend_tpu_integration():
    ...
    output = jax.jit(
        lambda qkv, cu_seqlens: backend(
            qkv,
            qkv,
            qkv,
            VisionAttentionMetadata(cu_seqlens),
        )
    )(qkv, cu_seqlens)
...
def test_qwen3_metadata_is_host_planned_and_bucket_stable():
    ...
    *inputs, placements = visual._batch_items(first)
    features, pos_indices, pos_weights, position_ids, metadata = inputs

    assert placements == ((0, 0, 2),)
    assert [
    ...
    _assert_no_grid_layout_planning(jax.make_jaxpr(visual)(*inputs))

    jax.block_until_ready(visual.encode(*inputs))
    cache_size = visual.encode._cache_size()
    *second_inputs, _ = visual._batch_items(_items([(1, 2, 4)], [(0, 2)]))
    assert not np.array_equal(
        np.asarray(metadata.cu_seqlens),
        np.asarray(second_inputs[-1].cu_seqlens),
    )
    jax.block_until_ready(visual.encode(*second_inputs))
