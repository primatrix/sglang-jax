...
from sgl_jax.srt.multimodal.model_executor.diffusion.diffusion_model_runner import DiffusionModelRunner
...
class TestDiffusionPrecision(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            cls.negative_prompt_embeds = np.load(os.path.join(cls.project_root, 'test_negative_prompt_embeds.npy'))
            cls.latents_in = np.load(os.path.join(cls.project_root, 'test_latents_in.npy'))
            cls.latents_out_expected = np.load(os.path.join(cls.project_root, 'test_latents_out.npy'))
        except FileNotFoundError as e:
            raise unittest.SkipTest(f"Data files missing: {e}")
    ...
    def test_01_config_comparison(self):
        ...
        np.testing.assert_allclose(
            jax_sigmas, torch_sigmas, rtol=1e-5, atol=1e-5,
            err_msg="Sigmas mismatch!"
        )
    ...
    def test_02_step_by_step_comparison(self):
        ...
        prompt_embeds = np.squeeze(self.prompt_embeds, axis=1)  # (1, 1, 512, 4096) -> (1, 512, 4096)
        ...
        text_embeds = device_array(
            jnp.array(prompt_embeds_combined),
        ...
        latents = device_array(jnp.array(latents), sharding=NamedSharding(self.mesh, PartitionSpec()))
        ...
        for step_idx, step_data in enumerate(steps):
            ...
            torch_noise_cond = np.array(step_data['noise_pred_cond']['data']) if 'noise_pred_cond' in step_data else None
            torch_noise_uncond = np.array(step_data['noise_pred_uncond']['data']) if 'noise_pred_uncond' in step_data else None

            # Compare latents_before (should match if previous step was correct)
            jax_latents_before = np.transpose(np.asarray(latents), (0, 4, 1, 2, 3))  # to (B, C, T, H, W)

            # BF16 has limited precision, use relaxed tolerance
            is_close, max_diff, _ = compare_arrays(
                f"Step {step_idx} latents_before",
                jax_latents_before, torch_latents_before,
                rtol=0.05, atol=0.1
            )
            ...
            latents_cf = latents_for_model.transpose(0, 4, 1, 2, 3)  # (B, T, H, W, C) -> (B, C, T, H, W)
            ...
            if do_classifier_free_guidance:
                ...
                if torch_noise_cond is not None:
                    compare_arrays(
                        ...
                        np.asarray(noise_cond), torch_noise_cond,
                        rtol=0.05, atol=0.1
                    )
                if torch_noise_uncond is not None:
                    compare_arrays(
                        f"Step {step_idx} noise_pred_uncond",
                        np.asarray(noise_uncond), torch_noise_uncond,
                        rtol=0.05, atol=0.1
                    )
            else:
            ...
            is_close_noise, max_diff_noise, _ = compare_arrays(
                ...
                np.asarray(noise_pred_final), torch_noise_pred,
                rtol=0.05, atol=0.1
            )
            ...
            is_close_after, max_diff_after, _ = compare_arrays(
                ...
                jax_latents_after, torch_latents_after,
                rtol=0.05, atol=0.1
            )
    ...
    def test_03_final_output_comparison(self):
        ...
        np.testing.assert_allclose(
            latents_out, self.latents_out_expected,
            rtol=0.05, atol=0.1,
