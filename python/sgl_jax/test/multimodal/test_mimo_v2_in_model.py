...
def _vision_config(**overrides: object) -> SimpleNamespace:
    values = {
        "patch_size": 1,
...
def _audio_config(**overrides: object) -> SimpleNamespace:
    values = {
        "audio_channels": 2,
        "group_size": 4,
...
def _item(
    ...
    placeholder_range: tuple[int, int],
    **metadata: object,
) -> MultimodalDataItem:
...
def test_composite_plan_packs_vision_and_audio_codes() -> None:
    image = _item(
        Modality.IMAGE,
...
def test_video_uses_shared_vision_tower() -> None:
    video = _item(
        Modality.VIDEO,
...
def test_tiny_vision_and_audio_towers_run_on_fixed_shapes() -> None:
    mesh = _mesh()
    with jax.set_mesh(mesh):
        ...
        vision_meta = MiMoV2VisionMetadata(
...
def test_audio_placeholder_expansion_refreshes_vision_ranges() -> None:
    ...
    assert audio.feature is not None
    assert audio.feature.shape == (8, 2)
    assert audio.placeholder_ranges == [(0, 2)]


def test_backbone_consumes_materialized_embeddings_for_extend() -> None:
    mesh = _mesh()
    ...
    with jax.set_mesh(mesh):
        model = MiMoV2Model(cast(PretrainedConfig, config), mesh, dtype=jnp.float32)
...
def test_multimodal_wrappers_compose_with_both_backbones(
    model_type: type[MiMoV2ForConditionalGeneration] | type[MiMoV2FlashForConditionalGeneration],
) -> None:
    mesh = _mesh()
    config = SimpleNamespace(
        vocab_size=8,
    ...
    with jax.set_mesh(mesh):
        model = model_type(cast(PretrainedConfig, config), mesh=mesh, dtype=jnp.float32)
...
def test_processor_normalizes_dict_vision_config() -> None:
    config = SimpleNamespace(
    ...
    processor = MiMoV2Processor(
        ...
        cast(ServerArgs, SimpleNamespace(model_path=".")),
        cast(ProcessorMixin, SimpleNamespace()),
    )
...
def test_processor_skips_mrope_for_text_only_vision_request() -> None:
    config = SimpleNamespace(
        architectures=["MiMoV2ForConditionalGeneration"],
        vision_config=_vision_config(),
        audio_config=_audio_config(),
    ...
    processor = MiMoV2Processor(
        cast(PretrainedConfig, config),
        cast(ServerArgs, SimpleNamespace(model_path=".")),
        cast(ProcessorMixin, hf_processor),
    )
    output = asyncio.run(
        processor.process_mm_data_async(
            None,
            "hello",
            cast(GenerateReqInput, SimpleNamespace(video_data=None, audio_data=None)),
        )
    )
...
def test_model_config_selects_multimodal_wrapper(source: str, target: str) -> None:
