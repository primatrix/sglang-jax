...
class TestDummyBatch(unittest.TestCase):
    ...
    def test_warm_multimodal_merge_uses_runtime_embedding_layout(self):
        ...
        with patch.object(
            embedding_pipeline,
            "dp_local_merge",
            wraps=embedding_pipeline.dp_local_merge,
        ) as merge:
        ...
        assert merge.call_args_list[0].args[1].source.dtype == jnp.float32
        assert merge.call_args_list[1].args[1].deepstack.dtype == jnp.float32
        np.testing.assert_array_equal(forward_batch.input_embedding, expected)
