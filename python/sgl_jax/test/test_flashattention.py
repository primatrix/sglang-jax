...
class TestAttention(CustomTestCase):
    ...
    def run_test(
        ...
        # Shard q/extend_k/extend_v with P(None, "tensor") to match production in_specs
        sharding = NamedSharding(mesh, P(None, "tensor"))
        q_shard = jax.device_put(q, sharding)
        extend_k = jax.device_put(extend_k, sharding)
        extend_v = jax.device_put(extend_v, sharding)
