...
from sgl_jax.srt.disaggregation.host_ip import (
    _REJECTED_HOSTNAMES,
    resolve_host_ip,
)
...
def test_raises_when_all_strategies_fail(monkeypatch):
    ...
    with mock.patch(
        ...
        with pytest.raises(RuntimeError, match="resolve a usable host IP"):
            resolve_host_ip()
...
def test_rejected_hostnames_set_is_what_we_expect():
    assert "localhost" in _REJECTED_HOSTNAMES
