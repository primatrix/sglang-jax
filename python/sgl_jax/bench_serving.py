...
async def async_request_openai_chat_completions(
    ...
    request_time_stats_sample_rate = float(getattr(args, "request_time_stats_sample_rate", 0.0))
    if request_time_stats_sample_rate > 0 and args.backend != "sglang-oai-chat":
        raise ValueError(
            "--request-time-stats-sample-rate is only supported by " "--backend sglang-oai-chat"
        )
    rid = None
    ...
    collect_request_time_stats = bool(
        rid is not None and should_sample_request(rid, request_time_stats_sample_rate)
    )
    ...
    async with _create_bench_client_session(trace_configs=trace_configs) as session:
        ...
        if output.request_time_stats is not None:
            output.request_time_stats["timestamps_ns"]["client_request_start_ns"] = time.time_ns()
        output.start_time = st
        most_recent_timestamp = st
        try:
            async with session.post(url=api_url, json=payload, headers=headers) as response:
        ...
        if output.request_time_stats is not None:
            output.request_time_stats["timestamps_ns"]["client_request_done_ns"] = time.time_ns()
...
async def benchmark(
    ...
    sampled_request_time_stats = [
        output.request_time_stats for output in outputs if output.request_time_stats is not None
    ]
