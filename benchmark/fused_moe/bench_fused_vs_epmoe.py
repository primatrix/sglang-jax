...
Example usage:
    ...
    python benchmark/fused_moe/bench_fused_vs_epmoe.py \
        --model-path Qwen/Qwen2.5-MoE-A2.7B \
        --dist-init-addr 10.0.0.1:12345 --nnodes 2 --node-rank 0 \
        --ep-size 16
"""
...
def create_mesh(ep_size: int, tp_size: int) -> jax.sharding.Mesh:
    """
    Create JAX mesh for MoE execution.
    ...
    Returns:
        JAX mesh with (expert, tensor) axes
    """
    world_size = ep_size * tp_size
    devices = jax.devices()

    if world_size > len(devices):
        raise ValueError(f"ep_size * tp_size ({world_size}) exceeds device count ({len(devices)})")

    devices_array = np.array(devices[:world_size]).reshape(ep_size, tp_size)
...
def main():
    ...
    # Create mesh
    print(f"\nCreating JAX mesh: ep_size={args.ep_size}, tp_size={args.tp_size}")
    mesh = create_mesh(args.ep_size, args.tp_size)
