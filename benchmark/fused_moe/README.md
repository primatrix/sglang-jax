...
- `ep_size`, `tp_size`: Expert and tensor parallel sizes
...
### Error: "ep_size * tp_size exceeds device count"

Reduce `--ep-size` or `--tp-size` to fit available devices. Check available devices with:
...
**Note**: 4 chips cannot support `ep_size=8`. Recommended comparison: `ep_size=4` (full EP) vs `ep_size=1` (full TP).

**Test commands:**

```bash
# Test 1: EP=4, TP=1 (Expert Parallelism)
python benchmark/fused_moe/bench_fused_vs_epmoe.py \
...
# Test 2: EP=1, TP=4 (Tensor Parallelism only)
python benchmark/fused_moe/bench_fused_vs_epmoe.py \
    --model-path Qwen/Qwen3-Coder-30B-A3B-Instruct \
    --ep-size 1 \
    --tp-size 4 \
...
```bash
# Test 1: EP=8, TP=4 (Expert Parallelism)
# Machine 0 (rank 0):
python benchmark/fused_moe/bench_fused_vs_epmoe.py \
...
# Test 2: EP=1, TP=32 (Tensor Parallelism only)
# Machine 0 (rank 0):
python benchmark/fused_moe/bench_fused_vs_epmoe.py \
    --model-path /path/to/grok2 \
    --ep-size 1 \
