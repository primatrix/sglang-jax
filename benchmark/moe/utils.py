...
_ZERO_CROP_SIDE_EFFECT = os.environ.get("SGL_BENCH_ZERO_CROP", "0") == "1"

# Side-effecting ZeroCrop requires explicit sharding on TPU; most MoE benches
# already keep outputs live, so default to a no-op. Enable explicitly if needed.
if _IS_TPU and _ZERO_CROP_SIDE_EFFECT:
    try:
        from jax import ffi  # type: ignore

        def zero_crop(x: jax.Array) -> jax.Array:
            """Side-effecting no-op to keep preceding ops alive."""
            return ffi.ffi_call(
                "ZeroCrop",
