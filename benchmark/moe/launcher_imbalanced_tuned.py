#!/usr/bin/env python3
"""Tuned FusedEPMoE vs EPMoE under hotspot imbalance (half experts 2x load).

Tests the Round 2 tuned block configs against EPMoE baseline under the same
hotspot conditions as launcher_imbalanced.py (hotspot_count=128,
hotspot_ratio=0.6667 → each hot expert gets 2x load).

This fills the gap: prior imbalanced tests only used default FusedEPMoE config.
"""
import os
import sys
import time
...
from benchmark.moe.utils import MoEImbalanceSimulator, build_mesh  # noqa: E402
...
# ── Hotspot parameters (same as launcher_imbalanced.py) ──
HOTSPOT_COUNT = 128
HOTSPOT_RATIO = 0.6667
NON_HOTSPOT_ALPHA = 10000.0

# ── Token counts ──
TOKEN_COUNTS = [128, 256, 512, 1024, 2048, 4096, 8192]

# ── Tuned FusedEPMoE block configs (Round 2 winners) ──
...
FUSED_CONFIGS = [
    # R2 winner for small tokens (≤256)
    cfg("tuned_small", btc=16, bse=2048, bd2c=2048),
    # R2 winner for medium tokens (512-4096)
    cfg("tuned_medium", btc=16, bse=128),
    # R2 winner for large tokens (2048-8192)
    cfg("tuned_large", btc=16),
    # R1 base for reference (btc=32, all defaults)
    cfg("base_btc32"),
]


def generate_hotspot_logits(num_tokens):
    """Generate router logits with hotspot imbalance using MoEImbalanceSimulator."""
    ...
    logits = MoEImbalanceSimulator.create_logits_from_counts(
...
def run_epmoe(num_tokens, mesh, router_logits, quantization_config):
    ...
    with jax.set_mesh(mesh):
        ...
        times = multiple_iteration_timeit_from_trace(
            ...
            task=f"epmoe_hotspot_nt{num_tokens}",
            tries=ITERS,
            warmup=WARMUP,
        )
        if len(times) > 1:
...
def run_fused(config_name, block_config, num_tokens, mesh, router_logits, quantization_config):
    ...
    with jax.set_mesh(mesh):
        ...
        times = multiple_iteration_timeit_from_trace(
            ...
            data_generator=lambda: (),
            task=f"fused_hotspot_{config_name}_nt{num_tokens}",
            tries=ITERS,
            warmup=WARMUP,
        )
        if len(times) > 1:
...
def main():
    ...
    # Results: {num_tokens: {"epmoe": ms, config_name: ms, ...}}
    all_results = {}
    total_runs = len(TOKEN_COUNTS) * (1 + len(FUSED_CONFIGS))  # EPMoE + each fused config
    run_idx = 0

    if proc == 0:
        print(f"\n{'#'*70}")
        print("  Tuned FusedEPMoE vs EPMoE — Hotspot Imbalance")
        print(
            f"  hotspot_count={HOTSPOT_COUNT}, hotspot_ratio={HOTSPOT_RATIO}, "
            f"non_hotspot_alpha={NON_HOTSPOT_ALPHA}"
        )
        print(
        ...
        print(
            f"  {1 + len(FUSED_CONFIGS)} backends × {len(TOKEN_COUNTS)} tokens = {total_runs} runs"
        )
    ...
    for num_tokens in TOKEN_COUNTS:
        ...
        # Generate hotspot logits — same for all backends
        logits = generate_hotspot_logits(num_tokens)

        # --- EPMoE baseline ---
        run_idx += 1
        if proc == 0:
            print(f"\n  [{run_idx}/{total_runs}] EPMoE (baseline)")
        try:
    ...
    if proc == 0:
        ...
        for num_tokens in TOKEN_COUNTS:
            ...
            # Find best fused config for this token count
        ...
        # Also print balanced reference for comparison
        print("\n  For reference, balanced results from PROCESS_LOG Section 10:")
        print("    Balanced geo-mean: 1.23x (Fused wins)")
        print("    Compare with hotspot geo-mean above to quantify imbalance impact")
