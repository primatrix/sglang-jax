#!/usr/bin/env python3
"""Block-128 quantization: Tuned FusedEPMoE vs EPMoE — Hotspot imbalance.

Both backends use weight_block_size=[128, 128] (FP8 block-128 quantization).
Tests the tuned block configs against EPMoE baseline under hotspot conditions.
"""
import os
import sys
import time
...
def run_epmoe(num_tokens, mesh, router_logits, quantization_config):
    ...
    with jax.set_mesh(mesh):
        ...
        times = multiple_iteration_timeit_from_trace(
            ...
            task=f"epmoe_b128_hot_nt{num_tokens}",
            tries=ITERS,
            warmup=WARMUP,
        )
        if len(times) > 1:
...
def run_fused(config_name, block_config, num_tokens, mesh, router_logits, quantization_config):
    ...
    with jax.set_mesh(mesh):
        ...
        times = multiple_iteration_timeit_from_trace(
            ...
            data_generator=lambda: (),
            task=f"fused_b128_hot_{config_name}_nt{num_tokens}",
            tries=ITERS,
            warmup=WARMUP,
        )
        if len(times) > 1:
...
def main():
    ...
    if proc == 0:
        print(f"\n{'#'*70}")
        print("  Block-128 Quantization: Tuned FusedEPMoE vs EPMoE — Hotspot")
