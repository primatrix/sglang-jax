#!/usr/bin/env python3
"""Block-256 quantization: Tuned FusedEPMoE vs EPMoE — Hotspot imbalance.

Re-run with device-side timing extraction (device_duration_ps from profiler trace).
EPMoE uses per-channel quantization (weight_block_size=None).
FusedEPMoE uses block-256 quantization (subc_quant_wsz=256, default).
"""
import os
import sys
import time
...
# ── Tuned FusedEPMoE block configs ──
...
def generate_hotspot_logits(num_tokens):
    """Generate router logits with hotspot imbalance."""
...
def run_epmoe(num_tokens, mesh, router_logits, quantization_config):
    """Benchmark EPMoE (per-channel) and return mean latency in ms."""
    with jax.set_mesh(mesh):
        ...
        times = multiple_iteration_timeit_from_trace(
            ...
            task=f"epmoe_b256_hot_nt{num_tokens}",
            tries=ITERS,
            warmup=WARMUP,
        )
        if len(times) > 1:
...
def run_fused(config_name, block_config, num_tokens, mesh, router_logits, quantization_config):
    """Benchmark FusedEPMoE (block-256) with a specific block config."""
    with jax.set_mesh(mesh):
        tokens = jnp.empty((num_tokens, HIDDEN), dtype=DTYPE)
        ...
        times = multiple_iteration_timeit_from_trace(
            ...
            data_generator=lambda: (),
            task=f"fused_b256_hot_{config_name}_nt{num_tokens}",
            tries=ITERS,
            warmup=WARMUP,
        )
        if len(times) > 1:
...
def main():
    ...
    # EPMoE: per-channel (no weight_block_size)
    epmoe_qconfig = QuantizationConfig(moe_weight_dtype=WEIGHT_DTYPE, moe_activation_dtype=None)
    # FusedEPMoE: block-256 (default subc_quant_wsz=256)
    fused_qconfig = QuantizationConfig(moe_weight_dtype=WEIGHT_DTYPE, moe_activation_dtype=None)
    ...
    if proc == 0:
        print(f"\n{'#'*70}")
        print("  Block-256 Quantization: Tuned FusedEPMoE vs EPMoE — Hotspot")
        print(
    ...
    for num_tokens in TOKEN_COUNTS:
        ...
        # --- EPMoE baseline (per-channel) ---
        run_idx += 1
        if proc == 0:
            print(f"\n  [{run_idx}/{total_runs}] EPMoE (per-channel baseline)")
        try:
            ms = run_epmoe(num_tokens, mesh, logits, epmoe_qconfig)
        ...
        # --- Tuned FusedEPMoE configs (block-256) ---
        for config_name, block_config in FUSED_CONFIGS:
            ...
            try:
                ms = run_fused(config_name, block_config, num_tokens, mesh, logits, fused_qconfig)
