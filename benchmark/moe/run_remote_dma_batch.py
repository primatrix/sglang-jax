...
if [ -n "$(git status --porcelain)" ]; then
  git stash push -u -m codex-before-remote-dma-batch-bench >/dev/null
fi
git fetch {remote_name} {shlex.quote(ref)}
git switch -C remote-dma-batch-bench FETCH_HEAD
PYTHONPATH=$PWD/python /tmp/tpu_logs/venv/bin/python -m py_compile \
  benchmark/moe/bench_remote_dma_batch.py benchmark/moe/run_remote_dma_batch.py
...
def run_group(args: argparse.Namespace, group_name: str, out_dir: Path, run_id: str) -> None:
    ...
    for rank in list(range(1, len(pods))) + [0]:
        pod = pods[rank]
        remote_log = f"/tmp/remote_dma_batch_{run_id}_{group_name}_rank{rank}.log"
...
/tmp/tpu_logs/venv/bin/python -u -m benchmark.moe.bench_remote_dma_batch {cmd_args} > {remote_log} 2>&1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run pure remote DMA batch benchmark on pods.")
    ...
    parser.add_argument(
        "--modes", nargs="+", choices=["small_dma", "batch_dma"], default=["small_dma", "batch_dma"]
    )
    ...
    parser.add_argument("--trace-root", default="/tmp/remote_dma_batch_trace")
