#!/usr/bin/env python3
"""Block-256 quantization: EPMoE (per-channel) vs FusedEPMoE (block-256) — Balanced load.
...
# ── Tuned FusedEPMoE block configs (from v6e tuning) ──
# FP8 block-256: bd1c=512, bfc=256 (effective_for defaults)
...
def run_epmoe(num_tokens, mesh, router_logits, quantization_config):
    ...
    with jax.set_mesh(mesh):
        ...
        times = multiple_iteration_timeit_from_trace(
            ...
            task=f"epmoe_b256_nt{num_tokens}",
            tries=ITERS,
            warmup=WARMUP,
        )
        if len(times) > 1:
...
def run_fused(config_name, block_config, num_tokens, mesh, router_logits, quantization_config):
    ...
    with jax.set_mesh(mesh):
        ...
        times = multiple_iteration_timeit_from_trace(
            ...
            data_generator=lambda: (),
            task=f"fused_b256_{config_name}_nt{num_tokens}",
            tries=ITERS,
            warmup=WARMUP,
        )
        if len(times) > 1:
...
def main():
    ...
    if proc == 0:
        print(f"\n{'#'*70}")
        print("  Block-256 Quantization: EPMoE (per-ch) vs FusedEPMoE (block-256) — Balanced")
        print(
