...
def _estimate_vmem_bytes(
    ...
    # t_packing = _dtype_packing(dtype) # Unused variable
    local_num_tokens = case.num_tokens // case.ep_size
    padded_num_experts = ((case.num_experts + 127) // 128) * 128
    # padded_top_k = ((case.top_k + 127) // 128) * 128 # Unused variable
    ...
    if use_shared_expert:
        # [修改点] 使用 cfg.bse 而不是 bf
        # 1. SE Weights Double Buffering (b_se_w*_x2_vmem)
        # shape: (2, t_packing, bd/pack, bse)
        se_w1 = 2 * bd1 * cfg.bse * weight_bytes
...
def select_block_configs(
    ...
    *,
    ...
    bse_candidates: list[int] | None = None,  # [新增]
    ...
    def add(*, raw: FusedMoEBlockConfig, effective: FusedMoEBlockConfig) -> None:
        ...
        key = (
            ...
            effective.bd1c,
            effective.bd2c,
            effective.bse,  # [新增] Add bse to key
        )
        if key in seen:
            return
        seen.add(key)
