...
EXCLUDED_BLOCK_CONFIGS: dict[tuple[int, int, int, int, int, int], set[tuple[int, ...]]] = {
    (1024, 256, 8, 8192, 2048, 32): {
        (8, 16, 2048, 1024, 1024, 16, 2048, 1024, 1024, 2048),
        (8, 8, 2048, 1024, 1024, 8, 2048, 1024, 1024, 2048),
    },
    (4096, 256, 8, 8192, 2048, 32): {
        (8, 16, 2048, 1024, 1024, 16, 2048, 1024, 1024, 2048),
        (8, 8, 2048, 1024, 1024, 8, 2048, 1024, 1024, 2048),
    },
...
def _estimate_vmem_bytes(
    ...
    # Return the raw scratch estimate.  Conservative safety margins belong in
    # the caller-controlled `--tpu-vmem-estimate-scale`; applying an additional
    # hidden multiplier here makes block-config tuning reject valid candidates.
